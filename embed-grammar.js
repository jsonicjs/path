...
// Embeds path-grammar.jsonic into src/path.ts and go/path.go.
// Run via: npm run embed

const fs = require('fs')
const path = require('path')

const grammar = fs.readFileSync(path.join(__dirname, 'path-grammar.jsonic'), 'utf8')

const BEGIN = '// --- BEGIN EMBEDDED path-grammar.jsonic ---'
const END = '// --- END EMBEDDED path-grammar.jsonic ---'
...
const tsContent = grammar
  ...
  .replace(/\$\{/g, '\\${')
embed(
  path.join(__dirname, 'src', 'path.ts'),
  'const grammarText = `\n' + tsContent + '`'
)
...
embed(
  path.join(__dirname, 'go', 'path.go'),
  'const grammarText = `\n' + grammar + '`'
)
